...
class PicoGameBoy(ST7789):
    def __init__(self):
        self.__up = Pin(2, Pin.IN, Pin.PULL_UP)        # nc
        self.__down = Pin(3, Pin.IN, Pin.PULL_UP)      # nc
        self.__left = Pin(4, Pin.IN, Pin.PULL_UP)      # nc
        self.__right = Pin(5, Pin.IN, Pin.PULL_UP)     # nc
        self.__adc_vrx = ADC(26)
        self.__adc_vry = ADC(27)
        self.__sw_value = Pin(28, Pin.IN, Pin.PULL_UP)
        self.__button_A = Pin(6, Pin.IN, Pin.PULL_UP) # nc
        self.__button_B = Pin(7, Pin.IN, Pin.PULL_UP)  # nc
        self.__buzzer = PWM(Pin(15))
        super().__init__(width=240, height=240, id_=1, sck=14, mosi=15,
                         dc=12, rst=11, cs=13, bl=10, baudrate=62500000)
